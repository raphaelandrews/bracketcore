 ...
 */
export function convertToSingleElimination(de: DoubleEliminationBracket): SingleEliminationBracket {
 ...
 */
export function convertToDoubleElimination(se: SingleEliminationBracket): DoubleEliminationBracket {
  ...
  const grandFinal = finalRound?.matches[0] ? structuredClone(finalRound.matches[0]) : undefined;
 ...
 */
export function generateMatchIds(size: BracketSize): {
  upper: string[][];
  lower: string[][];
  grandFinal: string;
} {
  ...
  for (let r = 0; r < numUpperRounds; r++) {
    ...
    upper.push(Array.from({ length: matchesInRound }, (_, i) => `ub${r + 1}-${i + 1}`));
  }
...
export function buildBracketFlow(
  ...
  for (let r = 0; r < ids.upper.length; r++) {
    ...
    // Calculate which LB round this UB round's losers drop into
    // UB R1 losers → LB R1 (index 0)
    // UB R2 losers → LB R2 (index 1) - they join LB R1 winners
    // UB R3 losers → LB R4 (index 3) - they join LB R3 winners
    // UB R4 losers → LB R6 (index 5) - they join LB R5 winners
    // Formula: r === 0 ? 0 : (2 * r - 1)
    const lowerRoundIndex = r === 0 ? 0 : 2 * r - 1;
    const lowerRound = ids.lower[lowerRoundIndex];
    ...
    for (let i = 0; i < roundMatches.length; i++) {
      ...
      if (lowerRound && lowerRound[r === 0 ? Math.floor(i / 2) : i]) {
        // In LB R1, UB R1 losers pair up (2 losers per LB match)
        // In later LB rounds, UB losers join LB survivors 1:1 in slot 1
        const lbSlot = r === 0 ? ((i % 2) as 0 | 1) : (1 as const);
        const lbMatchIndex = r === 0 ? Math.floor(i / 2) : i;
        flow[matchId].loser = [lowerRound[lbMatchIndex], lbSlot];
      }
    }
  }
 ...
 */
export function buildBracketForSize(teams: Team[], size: BracketSize): DoubleEliminationBracket {
  const numUpperRounds = Math.log2(size);
  const ids = generateMatchIds(size);
  ...
  for (let r = 0; r < numUpperRounds; r++) {
    ...
    for (let i = 0; i < roundIds.length; i++) {
      if (r === 0) {
        ...
        matches.push(
          createMatch(roundIds[i], r, i, teams[teamAIndex] ?? null, teams[teamBIndex] ?? null),
        );
      } else {
  ...
  for (let r = 0; r < ids.lower.length; r++) {
    ...
    const roundName = r === ids.lower.length - 1 ? "LB Final" : `LB Round ${r + 1}`;
 ...
 */
export function handleByeAdvancement(bracket: DoubleEliminationBracket): DoubleEliminationBracket {
