...
import type { DoubleEliminationBracket, Match, Team } from "@bracketcore/registry";
import type {
  BracketSize,
  ValidationError,
  ScheduleConflict,
...
export function useBracketEditor() {
  ...
  const validationErrors = useMemo(() => validateBracket(bracket, bestOf), [bracket, bestOf]);

  const scheduleConflicts = useMemo(() => detectConflicts(bracket), [bracket]);

  const teamStats = useMemo(() => computeTeamStats(bracket), [bracket]);
  ...
  const pushHistory = useCallback(
    (newBracket: DoubleEliminationBracket, newTeams: Team[]) => {
      setHistory((prev) => {
  ...
  const handleTeamNameChange = useCallback(
    (teamId: string, name: string) => {
      const newTeams = teams.map((t) => (t.id === teamId ? { ...t, name } : t));
      setTeams(newTeams);
  ...
  const handleMatchStreamUrlChange = useCallback(
    (streamUrl: string) => {
      if (!selectedMatch) return;
      const updated = structuredClone(selectedMatch) as EditorMatch;
  ...
  const handleMatchVenueChange = useCallback(
    (venue: string) => {
      if (!selectedMatch) return;
      const updated = structuredClone(selectedMatch) as EditorMatch;
      updated.venue = venue;
  ...
  const handleBracketSizeChange = useCallback(
    (newSize: BracketSize) => {
  ...
  const handleRemoveTeam = useCallback(
    (teamId: string) => {
      ...
      const newSize = Math.pow(
        2,
        Math.ceil(Math.log2(Math.max(newTeams.length, 2))),
      ) as BracketSize;
      if (newSize !== bracketSize && [4, 8, 16, 32, 64].includes(newSize)) {
        setBracketSize(newSize as BracketSize);
      }
  ...
  const handleExport = useCallback(() => {
    const data = JSON.stringify({ teams, bracket, bestOf, connectorStyle, bracketSize }, null, 2);
