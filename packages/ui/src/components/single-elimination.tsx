import type { SingleEliminationBracket, Match } from "../types";
import { MatchCard } from "./match-card";
import { cn } from "../lib/cn";

export interface SingleEliminationProps {
  bracket: SingleEliminationBracket;
  className?: string;
  onMatchClick?: (match: Match) => void;
}

/**
 * Single elimination bracket with SVG connector lines.
 *
 * Uses CSS custom properties for sizing:
 * - `--bracket-match-width`  (default 13rem / 208px)
 * - `--bracket-match-height` (default 4.25rem / 68px)
 * - `--bracket-round-gap`    (default 3rem / 48px) — horizontal space between rounds
 * - `--bracket-match-gap`    (default 1rem / 16px)  — base vertical gap (round 0)
 *
 * Theming via CSS custom properties:
 * - `--bracket-bg`, `--bracket-card`, `--bracket-border`,
 *   `--bracket-fg`, `--bracket-muted`, `--bracket-accent`
 */
export function SingleElimination({
  bracket,
  className,
  onMatchClick,
}: SingleEliminationProps) {
  const rounds = bracket.rounds;

  return (
    <div
      className={cn(
        "bracket-single-elimination",
        "inline-flex overflow-x-auto",
        "rounded-lg p-6",
        "bg-[var(--bracket-bg,hsl(var(--background)))]",
        className
      )}
    >
      {rounds.map((round, roundIdx) => {
        const isLast = roundIdx === rounds.length - 1;
        return (
          <div key={round.name} className="flex">
            {/* Round column */}
            <RoundColumn
              roundIdx={roundIdx}
              name={round.name}
              matches={round.matches}
              onMatchClick={onMatchClick}
            />
            {/* Connector to next round */}
            {!isLast && (
              <ConnectorColumn
                roundIdx={roundIdx}
                sourceCount={round.matches.length}
              />
            )}
          </div>
        );
      })}
    </div>
  );
}

// ---------------------------------------------------------------------------
// Internal: Round column
// ---------------------------------------------------------------------------

function RoundColumn({
  roundIdx,
  name,
  matches,
  onMatchClick,
}: {
  roundIdx: number;
  name: string;
  matches: Match[];
  onMatchClick?: (match: Match) => void;
}) {
  // For round R the slot height doubles: slot_R = 2^R * baseSlot
  // We use CSS calc via inline style so the user can override via custom properties.
  // baseSlot = matchHeight + matchGap
  // slotHeight = 2^R * baseSlot
  // gap between cards = slotHeight - matchHeight
  // topPadding = (slotHeight - matchHeight) / 2   (to center with connector midpoint)

  const exp = Math.pow(2, roundIdx);

  return (
    <div className="flex flex-col items-center shrink-0">
      <div className="text-xs font-medium text-[var(--bracket-muted,hsl(var(--muted-foreground)))] mb-3 whitespace-nowrap">
        {name}
      </div>
      <div
        className="flex flex-col"
        style={
          {
            "--_base":
              "calc(var(--bracket-match-height, 4.25rem) + var(--bracket-match-gap, 1rem))",
            "--_slot": `calc(${exp} * var(--_base))`,
            "--_gap":
              "calc(var(--_slot) - var(--bracket-match-height, 4.25rem))",
            "--_pad": "calc(var(--_gap) / 2)",
            gap: "var(--_gap)",
            paddingTop: roundIdx > 0 ? "var(--_pad)" : undefined,
          } as React.CSSProperties
        }
      >
        {matches.map((match) => (
          <div
            key={match.id}
            style={
              {
                height: "var(--bracket-match-height, 4.25rem)",
              } as React.CSSProperties
            }
          >
            <MatchCard
              match={match}
              onMatchClick={onMatchClick}
              className="h-full"
            />
          </div>
        ))}
      </div>
    </div>
  );
}

// ---------------------------------------------------------------------------
// Internal: SVG connector column between two rounds
// ---------------------------------------------------------------------------

function ConnectorColumn({
  roundIdx,
  sourceCount,
}: {
  roundIdx: number;
  sourceCount: number;
}) {
  // Each source match pair connects to one target match.
  // We render an SVG per pair, stacked vertically.

  const pairCount = Math.floor(sourceCount / 2);
  const exp = Math.pow(2, roundIdx);
  // A pair spans 2 source slots. 1 source slot = 2^roundIdx * baseSlot.
  // So pair height = 2 * sourceSlot = 2^(roundIdx+1) * baseSlot.
  // Top padding for this column = same as source round's top padding = (sourceSlot - matchHeight) / 2.

  return (
    <div
      className="flex flex-col shrink-0"
      style={
        {
          "--_base":
            "calc(var(--bracket-match-height, 4.25rem) + var(--bracket-match-gap, 1rem))",
          "--_src-slot": `calc(${exp} * var(--_base))`,
          "--_pair-h": `calc(${exp * 2} * var(--_base))`,
          "--_pad":
            roundIdx > 0
              ? "calc((var(--_src-slot) - var(--bracket-match-height, 4.25rem)) / 2)"
              : "0px",
          width: "var(--bracket-round-gap, 3rem)",
          paddingTop: "var(--_pad)",
        } as React.CSSProperties
      }
    >
      {Array.from({ length: pairCount }, (_, i) => (
        <svg
          key={i}
          className="w-full text-[var(--bracket-border,hsl(var(--border)))]"
          style={{ height: `var(--_pair-h)` }}
          viewBox="0 0 100 100"
          preserveAspectRatio="none"
          aria-hidden
        >
          {/*
            viewBox: 0 0 100 100 (stretched to actual pixel size).
            Top match center = 25%  (1/4 of pair height)
            Bot match center = 75%  (3/4 of pair height)
            Target center    = 50%
            Draw:
              top horizontal  -> vertical -> mid horizontal
              bot horizontal  -> vertical
          */}
          <path
            d={[
              // top match → right
              "M 0 25 H 50",
              // bottom match → right
              "M 0 75 H 50",
              // vertical between top and bottom
              "M 50 25 V 75",
              // midpoint → target
              "M 50 50 H 100",
            ].join(" ")}
            fill="none"
            stroke="currentColor"
            strokeWidth={1.5}
            vectorEffect="non-scaling-stroke"
          />
        </svg>
      ))}
    </div>
  );
}
